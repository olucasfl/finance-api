...
**Test coverage**: ~96% statements / ~85% branches / ~95% functions / ~97% lines as of this writing (`npm run test:cov`) — every module has real behavioral tests, not just "should be defined" scaffolding. Conventions used throughout, follow them for new specs: `PrismaService` is mocked as a plain object of `jest.fn()`s covering only the methods actually used (never a real DB); external HTTP (OpenAI, the liturgy API, Brevo, `ip-api.com`) is mocked at the module level (`jest.mock('axios')` or `global.fetch = jest.fn()`), never hitting the real network; assertions are state-based (what a function returns, what Prisma was called with), not interaction-only. `collectCoverageFrom` excludes `*.module.ts` and `main.ts` — pure NestJS DI wiring with no branching logic of its own.
...
- `BibleMark` / `BibleCollection` / `BibleCollectionItem` — the Bible study feature (highlights, favorites, per-verse notes, and named collections). A verse is identified by `book` (the Portuguese name from the frontend's static Bible JSON, same string the frontend routes use) + `chapter` + `verse`. `BibleMark` is **one row per marked verse** with independent `highlighted`/`favorite`/`note` — the service **deletes the row when all three are empty** (there is no "empty mark"), so the row existing is never proof of a specific state on its own. `reference` ("João 3,16") and `text` (the verse itself) are **snapshots the client sends on every write** — the backend has no Bible text (it lives only in the frontend bundle), and the text is static so the snapshot can't go stale. `BibleCollectionItem` carries the same snapshot fields plus its own optional `note` (separate from `BibleMark.note`). Deleting a collection cascades to its items; collection names are not unique.
...
- **`src/modules/users/`** — account CRUD: registration (`create`, gated by `X-App` like login), profile updates, account deletion. Registration checks `password === confirmPassword` server-side (not just a frontend concern) and sends the verification email through `MailService`. **`deleteAccount()` requires the current password** (`DeleteAccountDto`, checked with `bcrypt.compare` same as `changePassword`) before permanently deleting the user and everything cascaded from it — an access token alone (stealable via XSS, an unlocked device) used to be enough to trigger an irreversible deletion; now it isn't. **`setAdminStatus()`'s `ADMIN_PASSWORD` check is constant-time** (`matchesAdminPassword`, hashes both sides to SHA-256 then `crypto.timingSafeEqual`) rather than a plain `!==`, and short-circuits to `false` if `ADMIN_PASSWORD` itself isn't set — don't reintroduce a direct string comparison here, and don't let an unset env var "match" an empty candidate.
